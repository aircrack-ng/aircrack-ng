...
{
	uint32_t
		signature; /* signature (file magic) of .hccapx files, it is always the string HCPX */
   ...
   */
	...
	uint8_t
		keyver; /* set to 1 if WPA is used, other values (preferably 2) means WPA2  */
	uint8_t keymic
		[16]; /* the actual hash value (MD5 for WPA, SHA1 for WPA2) truncated to 128 bit (16 bytes) */
	uint8_t mac_ap[6]; /* BSSID */
	uint8_t
		nonce_ap[32]; /* nonce (random salt) generated by the access point */
	uint8_t mac_sta
		[6]; /* mac address of the client connecting to the access point */
	uint8_t nonce_sta
		[32]; /* nonce (random salt) generated by the client connecting to the access point */
