...
static bool ap_has_required_security(unsigned int ap_security,
									 unsigned int required_security)
{
	bool const has_required = ap_security == 0 || required_security == 0
	...
	unsigned long const min_packets)
{
	...
	if (!ap_has_required_security_and_essid(
			ap_cur, encryption_filter, essid_filter))
	{
		should_be_logged = false;
		goto done;
	}
