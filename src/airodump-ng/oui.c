...
static inline int OUI_ID_COMPARE(oui_id const * const a, oui_id const * const b)
{
	return memcmp((void *) a, (void *) b, sizeof *a);
}

static inline bool OUI_ID_EQUAL(oui_id const * const a, oui_id const * const b)
{
	return OUI_ID_COMPARE(a, b) == 0;
}

static inline void OUI_ID_COPY(oui_id * const dest, oui_id const * const src)
{
	*dest = *src;
}
...
static void oui_initialise(struct oui * const oui,
...
oui_context_st * load_oui_file(void)
{
	...
	while (fgets(buffer, sizeof(buffer), fp) != NULL)
	{
		...
		if (sscanf(buffer, "%2x-%2x-%2x", &a, &b, &c) == 3)
		{
			...
			oui_id const id = {.addr[0] = a, .addr[1] = b, .addr[2] = c};
...
static struct oui * oui_lookup(struct oui_list_head * const list,
							   oui_id const * const id)
{
	struct oui * oui;
...
char * get_manufacturer_by_oui(oui_context_st * const context,
