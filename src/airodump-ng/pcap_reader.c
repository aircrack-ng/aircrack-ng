...
static pcap_reader_result_t
packet_reader_80211(struct pcap_reader_context_st * const packet_reader_context,
					uint8_t * const packet_buffer,
					size_t const buffer_size,
					size_t * const packet_length,
					struct rx_info * const ri)
...
static pcap_reader_result_t
packet_reader_prism(struct pcap_reader_context_st * const packet_reader_context,
					uint8_t * const packet_buffer,
					size_t const buffer_size,
					size_t * const packet_length,
					struct rx_info * const ri)
	...
	struct rx_info * const ri)
{
	...
	if (ieee80211_radiotap_iterator_init(&iterator, rthdr, *packet_length, NULL)
		< 0)
	{
		result = pcap_reader_result_skip;
		goto done;
...
static pcap_reader_result_t
packet_reader_ppi(struct pcap_reader_context_st * const packet_reader_context,
				  uint8_t * const packet_buffer,
				  size_t const buffer_size,
				  size_t * const packet_length,
				  struct rx_info * const ri)
...
pcap_reader_result_t pcap_read(pcap_reader_context_st * const context,
							   ...
							   struct timeval * const packet_timestamp)
{
	...
	*packet_length = pkh.caplen;
	...
	result = context->packet_reader(
		context, packet_buffer, buffer_size, packet_length, ri);
...
pcap_reader_context_st * pcap_reader_open(char const * const filename)
{
	...
	if (fread(&context->pfh_in, 1, sizeof context->pfh_in, context->fp)
