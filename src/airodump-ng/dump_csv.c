			   ...
			   struct essid_filter_context_st const * const essid_filter)
{
	...
	TAILQ_FOREACH(ap_cur, ap_list, entry)
	{
		...
		{
			if (ap_cur->security & ENC_CCMP) fprintf(fp, " CCMP");
			...
			if (ap_cur->security & ENC_WEP) fprintf(fp, " WEP");
		...
		else
		{
			...
			if (ap_cur->security & AUTH_MGT) fprintf(fp, " MGT");
			if (ap_cur->security & AUTH_CMAC) fprintf(fp, " CMAC");
...
static void csv_context_free(struct csv_dump_context_st * const context)
{
	free(context);
}

static void csv_dump(void * const priv,
					 ...
					 struct essid_filter_context_st const * const essid_filter)
{
	...
	dump_write_csv(context->fp, ap_list, sta_list, f_encrypt, essid_filter);
}

static void csv_dump_close(struct csv_dump_context_st * const context)
{
	if (context == NULL)
	{
		goto done;
...
struct csv_dump_context_st * csv_dump_context_open(char const * const filename)
{
	bool had_error;
	struct csv_dump_context_st * context = calloc(1, sizeof *context);
...
bool csv_dump_open(struct dump_context_st * const dump,
