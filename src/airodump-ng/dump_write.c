...
int dump_write_airodump_ng_logcsv_add_ap(FILE * fp,
										 ...
										 float const * const gps_loc)
{
	...
	{
		if (ap_cur->security & STD_WPA2) fputs(" WPA2 ", fp);
		if (ap_cur->security & STD_WPA) fputs(" WPA ", fp);
		if (ap_cur->security & STD_WEP) fputs(" WEP ", fp);
		if (ap_cur->security & STD_OPN) fputs(" OPN", fp);
	}
...
int dump_write_airodump_ng_logcsv_add_client(FILE * const fp,
											 const struct AP_info * ap_cur,
...
void dump_write(struct dump_context_st * const dump,
...
struct dump_context_st * dump_open(dump_type_t const dump_type,
								   ...
								   bool const use_gpsd)
{
	...
	{
		case dump_type_csv:
			if (!csv_dump_open(dump, filename))
			{
				success = false;
				goto done;
			}
		...
		case dump_type_kismet_csv:
			if (!kismet_csv_dump_open(dump, filename))
			{
				success = false;
				goto done;
			}
		...
		case dump_type_kismet_netxml:
			if (!kismet_netxml_dump_open(
					dump, filename, airodump_start_time, use_gpsd))
			{
				success = false;
				goto done;
			}
