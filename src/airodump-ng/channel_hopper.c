...
{
	int ch, ch_idx = 0, card = 0, chi = 0, cai = 0, first = 1, again;
	int dropped = 0;
			...
			if (possible_channels[ch_idx] == invalid_channel)
			{
				...
				if (dropped >= chan_count)
				{
					...
					struct channel_hopper_data_st const hopper_data
						= {.card = card, .u.channel = ch};

					IGNORE_LTZ(
						write(data_write_fd, &hopper_data, sizeof hopper_data));
