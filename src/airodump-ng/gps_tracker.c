...
static void gps_locations_initialise(float * const coordinates,
...
static void * gps_tracker_thread(void * arg)
{
	...
	while (*gps_context->do_exit == 0)
	{
		...
		if (is_json > 0)
		{
            ...
            * protocol we speak. */
			...
			if (is_json)
			{
				...
				if (send(gpsd_sock, line, strlen(line), 0)
					!= (ssize_t) strlen(line))
				{
					continue;
				}
        ...
        * connected to an old-style gpsd. */
		...
		gps_locations_initialise(gps_context->gps_location,
								 sizeof(gps_context->gps_location));
		...
		while (*gps_context->do_exit == 0)
		{
			...
			if (seconds_since_last_update
				> gps_context->gps_valid_interval_seconds)
			{
				gps_locations_initialise(gps_context->gps_location,
										 sizeof(gps_context->gps_location));
			}
			...
			if (is_json)
			{
                ...
                */
				...
				{
					if (!(strptime(
							  data, "%Y-%m-%dT%H:%M:%S", &gps_context->gps_time)
						  == NULL))
					{
				...
				{
					gps_context->gps_location[gps_longitude]
						= strtof(data, NULL);
					if (errno == EINVAL || errno == ERANGE)
					{
				...
				{
					gps_context->gps_location[gps_longitude_error]
						= strtof(data, NULL);
					if (errno == EINVAL || errno == ERANGE)
					{
				...
				{
					gps_context->gps_location[gps_latitude_error]
						= strtof(data, NULL);
					if (errno == EINVAL || errno == ERANGE)
					{
				...
				{
					gps_context->gps_location[gps_altitude_error]
						= strtof(data, NULL);
					if (errno == EINVAL || errno == ERANGE)
					{
				...
				{
					gps_context->gps_location[gps_altitude]
						= strtof(data, NULL);
					if (errno == EINVAL || errno == ERANGE)
					{
			...
			else
			{
				...
				ret = sscanf(temp + 2,
							 "%f",
							 &gps_context->gps_location[gps_speed]); /* speed */
				...
				ret = sscanf(
					temp + 2,
					"%f",
					&gps_context->gps_location[gps_heading]); /* heading */
				...
				ret = sscanf(
					temp + 2,
					"%f",
					&gps_context->gps_location[gps_altitude]); /* altitude */
...
static void gps_tracker_initialise(gps_tracker_context_st * const gps_context,
								   FILE * const fp,
								   volatile int * do_exit)
{
	...
	gps_context->gps_valid_interval_seconds
		= default_gps_valid_interval_seconds;
	gps_context->fp = fp;
					   ...
					   volatile int * do_exit)
{
	...
	if (pthread_create(
			&gps_context->gps_tid, NULL, &gps_tracker_thread, gps_context)
		!= 0)
	{
