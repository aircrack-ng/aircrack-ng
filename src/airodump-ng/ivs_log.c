...
static bool ivs_log_data(FILE * const fp,
...
bool ivs_log_keystream(FILE * const fp,
...
bool ivs_log_wpa_hdsk(FILE * const fp,
					  mac_address const * const bssid,
					  mac_address * const previous_bssid,
					  void const * const data,
					  size_t const data_size)
{
	...
	if (!ivs_log_data(
			fp, bssid, previous_bssid, IVS2_WPA, data, data_size, NULL, 0))
	{
		success = false;
		goto done;
	}
...
bool ivs_log_essid(FILE * const fp,
				   ...
				   size_t const data_size)
{
	...
	if (!ivs_log_data(
			fp, bssid, previous_bssid, IVS2_ESSID, data, data_size, NULL, 0))
	{
		success = false;
		goto done;
	}
