...
static int dump_write_kismet_netxml_client_info(FILE * const fp,
												...
												bool const use_gpsd)
{
	...
	{
		manuf = sanitize_xml((unsigned char *) client->manuf,
							 strlen(client->manuf));
	}
	else
	{
	...
	struct essid_filter_context_st const * const essid_filter)
{
	...
	TAILQ_FOREACH(ap_cur, ap_list, entry)
	{
		...
		fprintf(
			fp, "\t\t\t<max-rate>%d.000000</max-rate>\n", ap_cur->max_speed);
		fprintf(fp, "\t\t\t<packets>%lu</packets>\n", ap_cur->nb_bcn);
		...
		else if (ap_cur->security & STD_WPA2 || ap_cur->security & STD_WPA)
		{
			if (ap_cur->security & ENC_TKIP)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+TKIP");
			...
			if (ap_cur->security & AUTH_PSK)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+PSK");
			if (ap_cur->security & AUTH_CMAC)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+PSK+CMAC");
			if (ap_cur->security & ENC_CCMP)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+AES-CCM");
			if (ap_cur->security & ENC_WRAP)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+AES-OCB");
			if (ap_cur->security & ENC_GCMP)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+GCMP");
			if (ap_cur->security & ENC_GMAC)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+GMAC");
			if (ap_cur->security & AUTH_SAE)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+SAE");
			if (ap_cur->security & AUTH_OWE)
				fprintf(fp, NETXML_ENCRYPTION_TAG, "\t\t\t", "WPA+OWE");
		}
		...
		TAILQ_FOREACH(st_cur, sta_list, entry)
		{
			...
			{
				dump_write_kismet_netxml_client_info(
					fp, st_cur, ++client_nbr, use_gpsd);
			}
		}
		...
		/* BSS Timestamp */
		fprintf(
			fp, "\t\t<bsstimestamp>%llu</bsstimestamp>\n", ap_cur->timestamp);
	...
	TAILQ_FOREACH(st_cur, sta_list, entry)
	{
			...
			|| MAC_ADDRESS_IS_BROADCAST(&st_cur->base->bssid))
		{
			...
			strncpy(
				first_time, ctime(&st_cur->first_seen), TIME_STR_LENGTH - 1);
			first_time[strlen(first_time) - 1] = 0; // remove new line
...
static void
kismet_netxml_context_free(struct kismet_netxml_dump_context_st * const context)
{
...
static void
kismet_netxml_dump(void * const priv,
...
static void
kismet_netxml_dump_close(struct kismet_netxml_dump_context_st * const context)
{
	if (context == NULL)
	{
		goto done;
...
struct kismet_netxml_dump_context_st *
kismet_netxml_dump_context_open(char const * const filename,
								char const * const airodump_start_time,
								bool const use_gpsd)
{
	bool had_error;
	struct kismet_netxml_dump_context_st * context = calloc(1, sizeof *context);
...
bool kismet_netxml_dump_open(struct dump_context_st * const dump,
							 ...
							 bool const use_gpsd)
{
	...
	struct kismet_netxml_dump_context_st * const context
		= kismet_netxml_dump_context_open(
			filename, airodump_start_time, use_gpsd);
