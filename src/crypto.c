...
static int is_arp(void * wh, size_t len)
{
...
static int is_wlccp(void * wh, size_t len)
{
	size_t wlccpsize = 58;
...
static int michael_remove_byte(struct Michael * mic, const unsigned char bytes[4])
{
	if (mic->nBytesInM == 0)
	{
		// Clear the buffer
