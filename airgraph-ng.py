...
def dot_create(info):
	graph = ['digraph G {\nsize ="48,48";\nlabel="Generated by Airgraph-ng";\noverlap=scale;'] #start the graphviz config file
	Clients = info[0]
	AP = info[1]	
	...
	ConClient = 0 # tracks the number of connected clients shown
	APClient = 0 # track the number of access points with clients connected
	...
	def CAPR_graphviz_label_client(client,label):
		...
		#ConClient += 1 #track number of clients
	def CAPR_graphviz_label_AP(bssid,essidi,channel):
		graph.extend(['\t','"',bssid,'"','[label="',bssid,'\\nEssid:',essid,'\\nChannel:',channel,'"];\n'])
		#APClient += 1 #track number of access points
	...
	for mac in (Clients):
		...
		if key[5] != "(notassociated)":
			if AP.has_key(key[5]): # does key look up in the Access point dictionary
				...
				CAPR_graphviz_label_AP(key[5],essid,bssidI[3])
