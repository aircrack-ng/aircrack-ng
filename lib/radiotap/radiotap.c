	...
	const struct ieee80211_radiotap_vendor_namespaces * vns)
{
	...
	if (max_length
		< get_unaligned_le16(UNALIGNED_ADDRESS(&radiotap_header->it_len)))
		return -EINVAL;
	...
	iterator->_bitmap_shifter
		= get_unaligned_le32(UNALIGNED_ADDRESS(&radiotap_header->it_present));
						 ...
						 int * size)
{
	...
	if (!iterator->overrides) return 0;
	...
	while (1)
	{
		...
		switch (iterator->_arg_index % 32)
		{
			...
			default:
				...
				{
					align = iterator->current_namespace
								->align_size[iterator->_arg_index]
								.align;
					size = iterator->current_namespace
							   ->align_size[iterator->_arg_index]
							   .size;
				}
				if (!align)
