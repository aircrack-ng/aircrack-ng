 ...
 */
static int get_taged_data( 	uint8_t *beacon_pkt,
							size_t pkt_len,
							uint8_t tag,
							uint8_t *tag_len,   /* Receives lengt of the data */
							uint8_t **tag_data )/* Receives pointer to data */
{
	size_t pos;
	size_t taglen;      /* Length of tag-data. Does not include the two-byte tag-header */
	uint8_t tagtype;

	if(memcmp(beacon_pkt, "\x80", 1)) {
		/* This was not a beacon-frame */
		return -1;
	}
...
int get_channel(uint8_t *beacon_pkt,
				size_t pkt_len)
{
	uint8_t *ch;
	uint8_t *ht_info;
	...
	if(get_taged_data(beacon_pkt, pkt_len, MGNT_PAR_CHANNEL, &tag_len, &ch ) == 0) {
		if(tag_len >= 1) {
			return *ch;
		}
	}
	...
	if(get_taged_data(beacon_pkt, pkt_len, MGNT_PAR_HT_INFO, &tag_len, &ht_info )) {
		/* tag not found... */
		return -1;
	}
					...
					char * essid)
{
	...
	while (1)
	{
		...
		if(get_taged_data(pkt_sniff, len, MGNT_PAR_SSID, &taglen, &data ))
			continue;
		...
		if (bssid != NULL
			...
			&& strncasecmp(essid, (char *) data, taglen)
				   == 0
