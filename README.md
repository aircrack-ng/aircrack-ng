...
# what the fork?
I created this very simple crappy fork to enhance the `airgraph-ng` script. It first replaces optparse with argparse. It also changes the behaviour of airgraph-ng so that it can keep, with `-d` option, the generated dot graphfile that is exported to a PNG image. It modifies the way the output filenames are autogenerated to distinguish *CPG* and *CAPR* ouputs. Finally, it adds the use of `AIRGRAPH_HOME` environment variable to get more flexibility with the installation.
